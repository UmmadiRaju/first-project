...
const agentId = 5284;
const profile = "projects/mic-aaa-preprod/conversationProfiles/hYi9Zm8HQ7CYjL8wmv7OEQ";
const appName = "mic-aaa-preprod";
const dialerServiceEndpoint = "https://dialeraaapreprod.micnxt.com/api/callinteractions";
const region = null;
let voicemail = false;
let is_pstn_transfer_enabled = true;
let outboundCallObj = null;
const axaDIDNumber = "+541143708247";
const dialogflowTTSVoice = "en-US-Standard-H";//Wavnet voice 
//const outboundDID = "61871002308"
const outboundDID = "18882693710"
...
VoxEngine.addEventListener(AppEvents.Started, function (ev) {
    ...
    contactDataId = dialinfo.contactDataId;
    
    //lokesh changes start
    let testJson = dialinfo.firstName.split("'").join("\"");
    //Logger.write( " : testJson :"+testJson );
    let dailogInfo = JSON.parse(testJson);

    dialinfo.contactDataId = contactDataId;
    // Logger.write( " : dailogInfo :"+dailogInfo );
    //Logger.write( " : contactNumber :"+dailogInfo.firstName);
    ...
    //dialinfo.PatientType = dailogInfo.PatientType;
    dialinfo.isHIPAAConsentProvided = dailogInfo.isHIPAAConsentProvided;
    dialinfo.month = dailogInfo.month;
    dialinfo.year = dailogInfo.year;
    //dialinfo.mobileNo = dailogInfo.mobileNo;
    dialinfo.toEmail = dailogInfo.toEmail;
    dialinfo.caseNumber = dailogInfo.caseNumber;
    dialinfo.patientReferenceNumber = dailogInfo.patientReferenceNumber;
    dialinfo.productName = dailogInfo.productName;
   //lokesh changes end
   
    first_name = dialinfo.firstName;
    Logger.write( " : toEmail :"+dialinfo.toEmail);
    ...
    call = VoxEngine.callPSTN(phone_number, outboundDID);
...
function createParticipant() {
    ...
    conversationParticipant = conversation.addParticipant({
        ...
        dialogflowSettings: {
           ...
           // model: DialogflowModel.PHONE_CALL,  Commented to check on floarian param
            //model: DialogflowModel.COMMAND_AND_SEARCH,
           // modelVariant: DialogflowModelVariant.USE_ENHANCED, Commented to check on floarian param
            model: "latest_short",
...
function apiCall(){
    ...
    Net.httpRequest(dialerServiceEndpoint, function(e) {
      if(e.code == 200) { 
        ...
        Logger.write("error:hereeee  " + e.error);
